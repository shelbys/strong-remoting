...
function SharedMethod(fn, name, sc, options) {
  ...
  this.sharedClass = sc;
  this.documented = options.documented;
  if (this.documented === undefined) {
    this.documented = fn.documented;
  }
  if (this.documented === undefined) {
...
SharedMethod.prototype.invoke = function(scope, args, remotingOptions, ctx, cb) {
  ...
  if (accepts) {
    var invalidArgs = [];
    var doesNotContain = function(enumList, arg) {
      if (Array.isArray(arg)) {
        for (var i = 0; i < arg.length; i++) {
          if (enumList.indexOf(arg[i].toString()) === -1) {
            invalidArgs.push(arg[i]);
          }
        }
        return invalidArgs.length > 0;
      } else {
        return enumList.indexOf(arg.toString()) === -1;
      }
    };
    ...
    for (var i = 0; i < accepts.length; i++) {
      ...
      var emptyArray = Array.isArray(uarg) && (uarg.length === 0);
      ...
      //Is the arg a valid value?
      if (desc.required || (SharedMethod.getType(uarg) !== 'undefined' && (!emptyArray))) {
        var enumValues = desc.enum || (desc.items ? desc.items.enum : null);
        if (enumValues && doesNotContain(enumValues, uarg)) {
          var invalidMsg = '';
          var invalidArgsLen = invalidArgs.length;
          if (invalidArgsLen < 2) {
            var msg = invalidArgsLen === 0 ? uarg : invalidArgs[0];
            invalidMsg = '\'' + msg + '\' is not one of: ';
          } else {
            invalidMsg = '\'' + invalidArgs.join(', ') + '\' are not one of: ';
          }
          var err = new Error(invalidMsg + enumValues.join(', '));
          err.statusCode = 422;
          return cb(err);
        }
      }
...
function coerceAccepts(uarg, desc) {
  ...
  if (targetTypeIsArray && Array.isArray(uarg)) {
    ...
    if (desc.required && uarg.length === 0) {
