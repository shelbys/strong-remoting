      ...
      if(actualType === 'string' && desc.type !== 'any' && actualType !== desc.type) {
        ...
        switch(targetType) {
          ...
          case 'date':
            ...
            var isError = false;
            var messageSuffix = ' must be a date';

            if (Array.isArray(uarg)) {
              messageSuffix = ' must contain dates';
              for (i in uarg) {
                uarg[i] = parseDate(uarg[i]);
                isError = isError || isNaN(uarg[i]);
              }
            } else {
              uarg = parseDate(uarg);
              isError = isNaN(uarg);
            ...
            if (isError) {
              var err = new Error(name + messageSuffix);
              err.statusCode = 422;
              return fn(err);
            }
          ...
          case 'number':
            var isError = false;
            var messageSuffix = ' must be a number';

            if (Array.isArray(uarg)) {
              messageSuffix = ' must contain numbers';
              for (i in uarg) {
          ...
          case 'boolean':
            ...
            var isError = false;
            var messageSuffix = ' must be a boolean';
            if (Array.isArray(uarg)) {
              messageSuffix = ' must contain booleans';
              for (i in uarg) {
                uarg[i] = parseBoolean(uarg[i]);
                isError = isError || isNaN(uarg[i]);
              }
            } else {
              uarg = parseBoolean(uarg);
              isError = isNaN(uarg)
            }
