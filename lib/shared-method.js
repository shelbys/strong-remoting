 ...
 * Constants.
 */
TYPES_PRIMITIVE = ['any', 'array', 'boolean', 'integer', 'number', 'object', 'string'];
...
SharedMethod.prototype.invoke = function (scope, args, fn) {
  ...
  if(accepts) {
    for(var i = 0; i < accepts.length; i++) {
      ...
      // validate arguments
      if (TYPES_PRIMITIVE.indexOf(targetType) != -1 || Array.isArray(desc.type)) {
        var schema = {};
        var keys = Object.keys(desc);

        for (key in keys) {
          schema[keys[key]] = desc[keys[key]];
        }

        if (Array.isArray(uarg)) { 
          schema.type = 'array';
          schema.items = {type: targetType};
        } else {
          schema.type = targetType;
        } 
          
        var validator = new jsonschema.Validator();
        if (desc.format && validator.getFormatNames().indexOf(desc.format) == -1) {
          delete(schema.format);
        }
          
        validator.addSchema(schema, name);
        validationResult = validator.validate(uarg, schema, { propertyName: name });

        log.fatal('ja schema');
        log.fatal(schema);
        log.fatal(desc);
        if ( validationResult.errors.length > 0 ) {
          var err = new Error(validationResult.errors[0].stack);
          err.statusCode = 400;
          return fn(err);
        }
      }
