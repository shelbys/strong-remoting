  ...
  if(accepts) {
    for(var i = 0; i < accepts.length; i++) {
      ...
      if ((desc.required || actualType !== 'undefined')) {
        var errFlag = 0;
        var err = new Error('\'' + uarg + '\' is not one of: ');

        if (desc.enum && !(desc.enum.indexOf(uarg) > -1)) {
          err.message += desc.enum.join(", ");
          errFlag = 1;
        }
        else if (desc.items && desc.items.enum && !(desc.items.enum.indexOf(uarg) > -1)) {
          err.message += desc.items.enum.join(", ");
          errFlag = 1;
        }
