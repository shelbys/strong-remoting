...
SharedMethod.prototype.invoke = function(scope, args, remotingOptions, ctx, cb) {
  ...
  if (accepts) {
    var invalidArgs = function(enumList, arg) {
      var doesNotContain = [];
      if (Array.isArray(arg)) {
        arg.forEach(function(element) {
          if (enumList.indexOf(element.toString()) === -1) {
            doesNotContain.push(element);
          }
        });
      } else if (enumList.indexOf(arg.toString()) === -1) {
        doesNotContain.push(arg);
      }
    ...
    for (var i = 0; i < accepts.length; i++) {
      ...
      if (desc.required || (SharedMethod.getType(uarg) !== 'undefined' && (!emptyArray))) {
        ...
        if (enumValues) {
          var invalidArguments = invalidArgs(enumValues, uarg);
          if (invalidArguments.length > 0) {
            var invalidMsg = '\'' + invalidArguments.join(', ') + '\' not one of: ' +
                enumValues.join(', ');
            var err = new Error(invalidMsg);
            err.statusCode = 422;
            return cb(err);
          }
        }
