...
SharedMethod.prototype.invoke = function(scope, args, remotingOptions, ctx, cb) {
  ...
  if (accepts) {
    var doesNotContain = function(enumList, arg) {
      var argInEnum = function(value, array) {
        return array.indexOf(value.toString()) === -1;
      };
      var result = [];
      if (Array.isArray(arg)) {
        result = arg.filter(function(item) {
          return argInEnum(item, enumList);
        });
      } else if (argInEnum(arg, enumList)) {
        result.push(arg);
      }
      return result;
    };
    ...
    for (var i = 0; i < accepts.length; i++) {
      ...
      if (desc.required || (SharedMethod.getType(uarg) !== 'undefined' && (!emptyArray))) {
        ...
        if (enumValues) {
          var invalidValues = doesNotContain(enumValues, uarg);
          if (invalidValues.length > 0) {
            var invalidMsg = '\'' + invalidValues.join(', ') + '\' not any of: ' +
                enumValues.join(', ');
