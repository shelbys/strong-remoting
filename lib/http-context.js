...
HttpContext.prototype.done = function () {
  ...
  if (!accepts) {
    var acceptInRequest = this.req.get('Accept');
    var indexInSupportedTypes = this.supportedTypes.indexOf(acceptInRequest.replace(/;?\s*charset=.+;?/i, ''));
    if (indexInSupportedTypes > -1) {
      accepts = this.supportedTypes[indexInSupportedTypes];
    }
  }
